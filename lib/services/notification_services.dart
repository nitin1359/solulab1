...
class NotificationServices {
  ...
  void handleMesssage(BuildContext context, RemoteMessage message) {
    if (kDebugMode) {
      print('In handle Messsage function');
    }
    if (message.data['type'] == 'text') {
      // redirect to new screen or take different action based on payload that we receive.
    }
  }

  Future<void> showNotification(RemoteMessage message) async {
    String? imageUrl = message.notification?.android?.imageUrl; // Here do not use final with this string to show new image from firebase every time.
